...
public class DbAdapterLocal implements DbAdapter {
	...
	private static final String DATABASE_CREATE = "create table measurements "
			...
			+ "latitude BLOB, "
			+ "longitude BLOB, measurement_time BLOB, throttle_position BLOB, rpm BLOB, speed BLOB, fuel_type BLOB, engine_load BLOB, fuel_consumption BLOB, intake_pressure BLOB, intake_temperature BLOB, short_term_trim_bank_1 BLOB, long_term_trim_bank_1 BLOB, maf BLOB);";
	...
	public ArrayList<Measurement> getAllMeasurements() {
		...
		Cursor c = mDb.query(DATABASE_TABLE, new String[] { KEY_ROWID,
				KEY_LATITUDE, KEY_LONGITUDE, KEY_TIME, KEY_THROTTLE, KEY_RPM,
				KEY_SPEED, KEY_FUELTYPE, KEY_ENGINELOAD, KEY_FUELCONSUMPTION,
				KEY_INTAKEPRESSURE, KEY_INTAKETEMPERATURE,
				KEY_SHORTTERMTRIMBANK1, KEY_LONGTERMTRIMBANK1, KEY_MAF }, null,
				null, null, null, null);
		...
		for (int i = 0; i < c.getCount(); i++) {
			...
			try {
				...
				measurement.setIntakePressure(Integer.valueOf(intakePressure)); // TODO:
																				// Could
																				// be
																				// a
																				// problem...
